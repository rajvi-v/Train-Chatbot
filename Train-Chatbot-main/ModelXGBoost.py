...
def plot_hyperparameter_tuning(results, label, out_dir):
    ...
    ax.set_title(f"XGBoost — Hyperparameter Tuning ({label})")
    ax.set_ylim(min(rmse_vals) * 0.95, max(rmse_vals) * 1.05)
    plt.tight_layout()
    path = out_dir / f"xgb_hyperparameter_tuning_{label.replace('->','_')}.png"
...
def plot_actual_vs_predicted(y_true, y_pred, label, out_dir):
    ...
    ax.set_title(f"XGBoost — Actual vs Predicted ({label})")
    ax.legend()
    plt.tight_layout()
    path = out_dir / f"xgb_actual_vs_predicted_{label.replace('->','_')}.png"
    plt.savefig(path, dpi=150)
    plt.close()
    print(f"    Saved: {path}")
...
def plot_residuals(y_true, y_pred, label, out_dir):
    ...
    ax.set_ylabel("Count")
    ax.set_title(f"XGBoost — Residuals Distribution ({label})")
    ax.legend()
    plt.tight_layout()
    path = out_dir / f"xgb_residuals_{label.replace('->','_')}.png"
    plt.savefig(path, dpi=150)
    plt.close()
    print(f"    Saved: {path}")
...
def plot_feature_importance(model, label, out_dir):
    ...
    plt.tight_layout()
    path = out_dir / f"xgb_feature_importance_{label.replace('->','_')}.png"
    plt.savefig(path, dpi=150)
    plt.close()
    print(f"    Saved: {path}")
...
def plot_error_by_stops(pairs_test, y_true, y_pred, label, out_dir):
    ...
    ax.set_title(f"XGBoost — Prediction Error by Stops to Destination ({label})")
    plt.tight_layout()
    path = out_dir / f"xgb_error_by_stops_{label.replace('->','_')}.png"
    plt.savefig(path, dpi=150)
    plt.close()
    print(f"    Saved: {path}")


def train_xgb_direction(label, files, station_order, output_path):
    print("\n" + "=" * 60)
    ...
    payload = {
        "model":          final_model,
        "model_name":     f"XGBoost (n_est={best_n_est}, depth={best_depth})",
        "direction":      label,
        "features":       FEATURES,
...
if not all_files:
    raise FileNotFoundError("No .xlsx files found in this folder.")
...
if wat2wey_files:
    train_xgb_direction(
        ...
        output_path   = "model_xgb_WAT2WEY.pkl",
    )
...
if wey2wat_files:
    train_xgb_direction(
        ...
        output_path   = "model_xgb_WEY2WAT.pkl",
    )
